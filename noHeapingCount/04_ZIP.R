#Program: 04_ZIP.R
#Developed by: BES
#Purpose: Simulate data similar to WIHS as ZIP distribution, analyze using MSM, PG, and DR in geex
#Created: 02.04.20, last updated 02.05.23
...
log.RR <- 0.5
  ...
  simdat1$SP0<-ifelse(simdat1$exczero0==1,0,rpois(num.part,simdat1$lambda0))
  simdat1$SP1<-ifelse(simdat1$exczero1==1,0,rpois(num.part,simdat1$lambda1))
  ...
  tryCatch(out_model  <- zeroinfl(SP ~ 1 + inc + druguse + sxex +  agevis10 | 1 + druguse + sxex +  agevis10, data = simdat2, dist = "poisson", link="logit"), error = function(e) { skip_to_next <<- TRUE})
  if(skip_to_next==FALSE) {    
  init.ZIP<-coef(out_model)
  mod.iv<-ML.ZIP(simdat2)
  ...
  tryCatch(PG.ests<-PG_est(simdat2, estfun_gf_ZIP, mod.iv, CM0.iv.pg, CM1.iv.pg, RR.iv.pg), error = function(e) { skip_to_next2 <<- TRUE})
  ...
  #call estimating function
  tryCatch(DR.ests<-DR_est(simdat2, estfun_dr_ZIP, inc ~ druguse + agevis10 + sxex, mod.iv, CM0.iv.dr, CM1.iv.dr, RR.iv.dr), error = function(e) { skip_to_next2 <<- TRUE})
  ...
  #call estimating function
  tryCatch(DR.ests.mw<-DR_est(simdat2, estfun_dr_ZIP, inc ~ agevis10 + sxex, mod.iv, CM0.iv.dr.mw, CM1.iv.dr.mw, RR.iv.dr.mw), error = function(e) { skip_to_next2 <<- TRUE})
  ...
  #calculate the initial values for geex
  #out_model2  <- zeroinfl(SP ~ 1 + inc + druguse + sxex +  agevis10 | 1 + druguse + sxex +  agevis10, data = simdat2, dist = "poisson", link="logit")
  #out_model1  <- zeroinfl.reparm(SP ~ 1 + inc + druguse + sxex +  agevis10 | 1 + druguse + sxex +  agevis10, data = simdat2, dist = "poisson", link="logit")
  tryCatch(out_model.ms  <- zeroinfl(SP ~ 1 + inc + sxex +  agevis10 | 1 + sxex +  agevis10, data = simdat2, dist = "poisson", link="logit"), error = function(e) { skip_to_next <<- TRUE})
  if(skip_to_next==FALSE) { 
  init.ZIP.ms<-coef(out_model.ms)
  mod.iv.mo<-ML.ZIP.ms(simdat2)
  ...
  #call estimating function
  tryCatch(PG.ests.mo<-PG_est(simdat2, estfun_gf_inc_ZIP, mod.iv.mo, CM0.iv.pg.mo, CM1.iv.pg.mo, RR.iv.pg.mo), error = function(e) { skip_to_next2 <<- TRUE})
  ...
  #call estimating function
  tryCatch(DR.ests.mo<-DR_est(simdat2, estfun_dr_inc_ZIP, inc ~ druguse + agevis10 + sxex, mod.iv.mo, CM0.iv.dr.mo, CM1.iv.dr.mo, RR.iv.dr.mo), error = function(e) { skip_to_next2 <<- TRUE})
  ...
  #call estimating function
  tryCatch(DR.ests.mb<-DR_est(simdat2, estfun_dr_inc_ZIP, inc ~ agevis10 + sxex, mod.iv.mo, CM0.iv.dr.mb, CM1.iv.dr.mb, RR.iv.dr.mb), error = function(e) { skip_to_next2 <<- TRUE})
